...
class PostgresDocPackageRepositoryTest {
  ...
  @Mock private DocPackageJpaRepository jpa;
  ...
  private static DocPackageEntity sampleEntity(String id) {
    return new DocPackageEntity(
        id, "team-a", "product-x", "1.0.0", List.of("tag1"), NOW, "README.md");
  }
  ...
  void findByTeamProductAndVersion_mapsEntityToDomain() {
    ...
    Optional<DocPackage> result =
        repository.findByTeamProductAndVersion("team-a", "product-x", "1.0.0");
  ...
  void findByTeam_delegatesAndMaps() {
    when(jpa.findByTeam("team-a", unpaged())).thenReturn(List.of(sampleEntity("pkg-1")));
  ...
  void findByTag_delegatesAndMaps() {
    when(jpa.findByTag("tag1", unpaged())).thenReturn(List.of(sampleEntity("pkg-1")));
