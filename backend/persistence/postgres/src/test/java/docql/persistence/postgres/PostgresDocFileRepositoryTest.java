...
class PostgresDocFileRepositoryTest {
    ...
    @Mock
    private DocFileJpaRepository jpa;
    ...
    void saveAll_delegatesAllEntitiesToJpa() {
        ...
        List<DocFile> files = List.of(
                sampleDomain("pkg-1", "README.md"),
                sampleDomain("pkg-1", "guide.md")
        );
    ...
    void findByPackageId_mapsAllEntitiesToDomain() {
        when(jpa.findByIdPackageId("pkg-1")).thenReturn(List.of(
                sampleEntity("pkg-1", "README.md"),
                sampleEntity("pkg-1", "guide.md")
        ));

        List<DocFile> result = repository.findByPackageId("pkg-1");

        assertThat(result).hasSize(2);
        assertThat(result).extracting(DocFile::path)
                .containsExactlyInAnyOrder("README.md", "guide.md");
    }
