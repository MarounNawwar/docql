...
/**
 * Embedded Lucene {@link SearchEngine}.
 * Uses an in-memory {@link ByteBuffersDirectory} by default.
...
public class LuceneSearchEngine implements SearchEngine {
    ...
    public void index(DocFile file, String team, String product, String version) {
        try (IndexWriter writer = new IndexWriter(directory, new IndexWriterConfig(analyzer))) {
            ...
            doc.add(new TextField  (FIELD_TITLE,      file.title()   != null ? file.title()   : "",       Field.Store.YES));
            doc.add(new TextField  (FIELD_CONTENT,    file.content() != null ? file.content() : "",       Field.Store.NO));
    ...
    public List<SearchResult> search(String query, List<String> tags, String team) {
        try (DirectoryReader reader = DirectoryReader.open(directory)) {
            ...
            MultiFieldQueryParser parser = new MultiFieldQueryParser(
                    new String[]{FIELD_TITLE, FIELD_CONTENT}, analyzer);
