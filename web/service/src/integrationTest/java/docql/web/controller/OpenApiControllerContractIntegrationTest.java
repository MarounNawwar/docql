...
@ContextConfiguration(classes = {PublishController.class, DiscoveryController.class, SearchController.class})
class OpenApiControllerContractIntegrationTest {
    ...
    void publish_endpointUsesGeneratedContractAndReturnsCreated() throws Exception {
        var requestDto = new PublishRequestDto("eng", "docs", "1.0.0", List.of("api"), "index.md", List.of());
        var publishRequest = new PublishRequest("eng", "docs", "1.0.0", List.of("api"), List.of(), "index.md");
        var pkg = new DocPackage("p1", "eng", "docs", "1.0.0", List.of("api"), Instant.parse("2026-01-01T00:00:00Z"), "index.md");
        var responseDto = new DocPackageDto("p1", "eng", "docs", "1.0.0", List.of("api"), Instant.parse("2026-01-01T00:00:00Z"), "index.md");
    ...
    void discovery_endpointUsesGeneratedContractAndReturnsList() throws Exception {
        ...
        when(discoveryService.listByTeam("eng")).thenReturn(List.of(pkg));
        when(webMapper.toDocPackageDto(pkg)).thenReturn(responseDto);

        mockMvc.perform(get("/packages").queryParam("team", "eng"))
